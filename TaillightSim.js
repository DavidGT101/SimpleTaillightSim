(() => {
  ...
  function clsAll(keepRed){segs.forEach(s=>{s.className='seg';s.style.transitionDuration='';if(keepRed)s.classList.add('on')});clearSigIcons();updateReverseLights()}
  ...
  function activateBrake(){
    ...
    const btn=$('#btnBrake');
    if(btn)btn.classList.add('brake-active');
  }
  ...
  function deactivateBrake(){
    ...
    const btn=$('#btnBrake');
    if(btn)btn.classList.remove('brake-active');
  }
  ...
  function toggleReverse(){
    ...
    if(state.reverseActive){
      ...
      if(btn)btn.classList.add('reverse-active');
    }else{
      segs.forEach(s=>s.classList.remove('reverse'));
      statusReverse.classList.remove('active','reverse');
      const btn=$('#btnReverse');
      if(btn)btn.classList.remove('reverse-active');
    }
  }
  ...
  function restoreRedDelayed(group){if(!state.on)return;setTimeout(()=>{group.forEach(s=>{s.classList.add('on');s.style.transitionDuration='.4s'});setTimeout(()=>{group.forEach(s=>s.style.transitionDuration='');updateReverseLights()},420)},CFG.RETURN_TO_RED_DELAY)}
  ...
  function activate(dir){
      ...
      if(state.signal===dir){
          state.signal='none';state.animToken++;state.iconToken++;clearSigIcons();return
      }
      clsAll(state.on);
      state.signal=dir;
  ...
  async function lockUnlock(){if(state.sysBusy)return;if(!state.on){state.on=true;await hazardFlash(2,false);await sleep(CFG.WELCOME_DELAY);await systemSequence('welcome')}else{state.on=false;await hazardFlash(1,true);await sleep(CFG.GOODBYE_DELAY);await systemSequence('goodbye')}}
  ...
  const setupTouchControls=()=>{
    $('#btnLock')?.addEventListener('click',()=>lockUnlock());
    $('#btnLeft')?.addEventListener('click',()=>activate('left'));
    $('#btnHazard')?.addEventListener('click',()=>activate('hazard'));
    $('#btnRight')?.addEventListener('click',()=>activate('right'));
    $('#btnMode')?.addEventListener('click',()=>changeMode());
    
    const btnBrake=$('#btnBrake');
    if(btnBrake){
      btnBrake.addEventListener('touchstart',e=>{e.preventDefault();activateBrake()});
      btnBrake.addEventListener('touchend',e=>{e.preventDefault();deactivateBrake()});
      btnBrake.addEventListener('mousedown',()=>activateBrake());
      btnBrake.addEventListener('mouseup',()=>deactivateBrake());
      btnBrake.addEventListener('mouseleave',()=>deactivateBrake());
    }
    ...
    if(btnReverse){
      ...
      btnReverse.addEventListener('mousedown',()=>{
        reverseTimer=setTimeout(()=>toggleReverse(),CFG.REVERSE_HOLD_MS);
      });
      btnReverse.addEventListener('mouseup',()=>{
        if(reverseTimer){clearTimeout(reverseTimer);reverseTimer=null}
      });
    }
  };
  ...
  function segClassCount(cls){let n=0;for(const s of segs){if(s.classList.contains(cls))n++}return n}
